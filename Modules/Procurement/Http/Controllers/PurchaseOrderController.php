...
use Modules\Procurement\Dao\Models\PurchaseOrderDetailProgress;
use Modules\Procurement\Dao\Repositories\VendorRepository;
use Modules\Procurement\Dao\Repositories\PurchaseOrderCreateRepository;
use Modules\Procurement\Dao\Repositories\PurchaseOrderProgressRepository;
use Modules\Procurement\Dao\Repositories\PurchaseOrderRepository;
use Modules\Sales\Dao\Repositories\OrderRepository;
use Modules\Sales\Http\Services\PurchaseOrderService;
...
class PurchaseOrderController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new PurchaseOrderRepository();
        }
        if (self::$detail == null) {
            self::$detail = new PurchaseOrderCreateRepository();
        }
    ...
    public function data(MasterService $service)
    {
        if (request()->isMethod('POST')) {
            $datatable = $service
                ->setRaw(['procurement_work_order_status'])
                ->setAction(
                    [
                        'update' => ['primary', 'edit'],
                        'show'   => ['success', 'show'],
            ...
            $datatable->editColumn('procurement_work_order_status', function ($field) {
                return Helper::createStatus([
                    'value'  => $field->procurement_work_order_status,
                    'status' => self::$model->status,
                ]);
            });
            $datatable->editColumn('procurement_work_order_date', function ($field) {
                return $field->procurement_work_order_date->toDateString();
            });
    ...
    public function survey(PurchaseOrderService $service)
    {
        $progress = new PurchaseOrderProgressRepository();
        if (request()->isMethod('POST')) {
